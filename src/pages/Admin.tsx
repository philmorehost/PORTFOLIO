...
export default function Admin() {
  ...
  const handleMagicGenerate = async () => {
    ...
    setEditingId(null);
    try {
      const data = await generateProjectPitch(url, title);
      
      // High-accuracy scan with viewport matching and interaction delay
      const autoScreenshot = `https://api.microlink.io/?url=${encodeURIComponent(url)}&screenshot=true&embed=screenshot.url&viewport.width=1920&viewport.height=1080&waitFor=3000`;
  ...
  if (!isAuthorized) {
    ...
    const checkPass = () => {
      if (adminPassAttempt === adminKey) {
        setIsAuthorized(true);
      } else {
        alert('DECRYPT_FAILED');
      }
    };

    return (
      <div className="min-h-[80vh] flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-white/5 border border-white/10 p-10 rounded-[20px] space-y-8 backdrop-blur-xl">
          <div className="text-center space-y-4">
            <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-sharp-orange/10 border border-sharp-orange/20 mb-4 group">
              <Lock className="w-8 h-8 text-sharp-orange transition-transform group-hover:scale-110" />
            </div>
            <h1 className="text-3xl font-black tracking-tighter uppercase italic text-glow-orange">Gatekeeper Active</h1>
            <p className="text-[11px] text-text-dim font-mono uppercase tracking-[0.2em]">Administrative Bypass Payload Required</p>
          </div>
          
          <div className="space-y-4">
            <div className="relative">
              <Key className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-sharp-orange" />
              <input 
                type="password"
                ...
                className="w-full bg-black/60 border border-white/10 rounded-xl py-4 pl-12 pr-4 outline-none focus:border-sharp-orange transition-all font-mono"
              />
            </div>
            <button 
              onClick={checkPass}
              className="w-full py-4 bg-sharp-orange text-black font-black rounded-xl uppercase tracking-[0.2em] italic text-sm hover:brightness-110 transition-all active:scale-95"
            >
              Initialize Sync
            </button>
          </div>
          ...
          <div className="pt-4 border-t border-white/5 text-center">
            <p className="text-[9px] font-mono text-text-dim uppercase mb-4 tracking-tighter">Secure environment: Ensure VITE_ADMIN_PASSWORD is set</p>
            <div className="flex justify-center gap-1">
              <div className="w-1 h-1 rounded-full bg-sharp-orange animate-pulse"></div>
              <div className="w-1 h-1 rounded-full bg-sharp-orange animate-pulse delay-75"></div>
              <div className="w-1 h-1 rounded-full bg-sharp-orange animate-pulse delay-150"></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white/5 border border-white/10 p-10 rounded-[12px] space-y-8">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="w-full bg-black/40 border border-white/10 rounded-lg py-3 pl-10 pr-4 focus:border-sharp-orange outline-none transition-all font-medium"
              />
            </div>
          </div>
        ...
        <div className="flex gap-4">
          <button 
            ...
            className="flex-1 py-5 bg-glossy-purple text-white font-black rounded-xl flex items-center justify-center gap-3 neon-purple hover:brightness-110 active:scale-95 transition-all disabled:opacity-50 disabled:scale-100 uppercase tracking-widest text-sm"
          >
